...
function luviana_mphb_customize_register( $wp_customize ) {
	$wp_customize->add_setting( 'luviana_room_type_header_layout', array(
		'default'           => '',
		'sanitize_callback' => 'luviana_sanitize_select'
	) );
	$wp_customize->add_control( 'luviana_room_type_header_layout', array(
		'type'      => 'select',
		'section'   => 'luviana_header_options',
		'label'     => esc_html__( 'Accommodation Header Layout', 'luviana' ),
		'priority'  => 99,
		'choices'   => array(
			''          => esc_html__( 'Default', 'luviana' ),
			'bg-image'  => esc_html__( 'Image on background', 'luviana' ),
		),
	) );
}
add_action( 'customize_register', 'luviana_mphb_customize_register' );

function luviana_filter_single_room_type_classes($classes) {

    if ( is_singular( 'mphb_room_type' ) ) {
        if ( 'bg-image' === luviana_get_room_type_header_layout() ) {
            $classes[] = 'header-layout-bg-image';
        }
    }

    return $classes;
}
add_filter('post_class', 'luviana_filter_single_room_type_classes');
